...
- [ ] **Step 1:** Test: `map_status("HT")` -> the configured `halftime_status`;
  `"1H"`/`"2H"` -> `"live"`; `"NS"` -> `"scheduled"`; `"FT"`/`"AET"`/`"PEN"` -> `"finished"`;
  unknown -> `"live"`.
- [ ] **Step 2:** Implement `map_status(short: str) -> str` with a dict + default, reading the
  halftime token from `CONFIG["feed"]["halftime_status"]`.
- [ ] **Step 3:** `pytest` that test. **Step 4:** Commit.
...
- [ ] **Step 1:** Test `lineups()` against captured JSON returns **startXI only** (22 rows),
...
- [ ] **Step 2:** Test `record(snapshot, minute)` then `snapshot_at(minute)`: stats from
  ...
  recorded <= minute.
- [ ] **Step 3:** Test `match_status_at`/`match_status` use `map_status` on
...
- [ ] **Step 1:** Test `live_plan(elapsed, status, clock)` returns
...
- [ ] **Step 2:** Implement using `HalfClock`: `w_cur = elapsed // window_minutes + 1` for
...
- [ ] **Step 1:** Test with a fake `Transport` returning captured snapshot JSON that
...
- [ ] **Step 2:** `pytest`. **Step 3:** Commit (skip if no change needed beyond a test).
...
- [ ] **Step 1:** Implement a `Screen` showing the live match clock and "waiting for minute
  ...
  lineups" (pre-draft).
- [ ] **Step 2:** `update(dt)`: accumulate dt; if no fetch in flight and (never polled or
...
- [ ] **Step 3:** `_poll()` coroutine: `snap = await feed_client.get_feed(fixture_id)`;
  `feed.record(snap)`; clear in-flight; reset accumulator. Guard exceptions -> keep last good,
  clear in-flight.
- [ ] **Step 4:** Launch desktop to confirm the screen renders (it will sit waiting against
...
- [ ] **Step 1:** `flow.start_live(app, fixture_id, sim_mode=False)`: build a `LiveFeed` +
...
- [ ] **Step 2:** Extend `Flow` so the window sequence is driven by `scored_windows` from the
...
- [ ] **Step 3:** Handle the half-over plan (empty scored list) -> `FinalScreen` titled
  "First Half Over" / Room.
- [ ] **Step 4:** Add `--live` to `src/main.py` -> `flow.start_live(app, CONFIG["live"]["fixture_id"])`.
- [ ] **Step 5:** `pytest` (engine untouched -> still green); launch `--live` to verify it
...
- [ ] **Step 1:** Full `pytest` green; `src/main.py` (demo) still launches; `--sim` still works.
- [ ] **Step 2:** Confirm golden rule: no pygame import in any `src/game/` file; tests touch
